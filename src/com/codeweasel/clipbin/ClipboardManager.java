...
public final class ClipboardManager
{
	...
	public static void setClipboardContents(Clip pClip, ClipboardOwner pClipboardOwner)
	{
		...
		case IMAGE:
			break;
		case FILE_LIST:
			FileListTransferable fileListTransferable = new FileListTransferable(pClip.getFileData());
	...
	public static Clip getClipboardContents()
	{
		...
		else if ((contents != null) && contents.isDataFlavorSupported(DataFlavor.javaFileListFlavor))
		{
			try
			{
				result = new Clip((List<File>) contents.getTransferData(DataFlavor.javaFileListFlavor));
			}
			catch (UnsupportedFlavorException | IOException ex)
			{
				System.out.println(ex);
...
class FileListTransferable implements Transferable
{
	...
	public FileListTransferable(List<File> pFiles)
	{
		this.fFiles = pFiles;
	}
